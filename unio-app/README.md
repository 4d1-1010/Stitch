...
| Path | Scope |
|---|---|
| `unio/` | Runtime binary (`unio-ui`). Win32 + D3D11 on Windows; Xlib + EGL + desktop GL on Linux. Verifies pasted licence tokens at runtime. |
| `licensing/` | Vendor key material (`dev/`) + the `unio-license-tool` CLI that signs tokens. Dev-side only — not shipped to end users. |
...
`unio::license_format` static library so the bytes the tool signs
...
For a whole-repo build (also produces `unio-pipe`), use the
repo-root `packaging/docker/build-linux.sh` instead.

## Issuing a dev licence

```bash
./dist/linux-x64/unio-license-tool \
