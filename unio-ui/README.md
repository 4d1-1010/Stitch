...
`unio/apps/` tree (kept in the repo until the port is
...
| Rendering | **D3D11** (Windows) + **OpenGL 3.3 via EGL** (Linux) + **Metal** (macOS, future) | Native per OS — enables zero-copy video preview from `unio-pipe`'s presenter texture. |
| C++ standard | C++20 | Matches `unio-pipe/`. |
...
```bash
cmake -S unio-ui -B unio-ui/build -DCMAKE_BUILD_TYPE=Release
cmake --build unio-ui/build -j
./unio-ui/build/unio-ui
```
...
Until step 5 is done, the Python tree at `unio/` is still the
runnable app (`python -m unio.apps.shell`).
