...
///   * Linux:   $XDG_CONFIG_HOME/unio/workspaces.json,
///              fallback to $HOME/.config/unio/workspaces.json
///   * Windows: %APPDATA%/unio/workspaces.json
...
std::string config_dir() {
    ...
    if (SHGetFolderPathA(nullptr, CSIDL_APPDATA, nullptr, 0, buf) == S_OK) {
        std::string path = std::string(buf) + "\\unio";
    ...
    if (xdg && xdg[0] != '\0') {
        ...
        if (!home) return {};
        base = std::string(home) + "/.config/unio";
    }
