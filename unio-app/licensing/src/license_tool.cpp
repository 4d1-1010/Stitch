/// @file main.cpp
/// @brief `unio-license-tool` — CLI that signs a licence token
...
using unio_ui::license::LicenseToken;
using unio_ui::license::Tier;
...
[[noreturn]] void usage() {
    std::fprintf(stderr,
        "unio-license-tool — sign a licence token\n\n"
        "Usage:\n"
        "  unio-license-tool --key <private.pem> --customer <id>\n"
...
int main(int argc, char** argv) {
    ...
    const std::vector<std::uint8_t> payload =
        unio_ui::license::codec::encode_payload(t);
    ...
    const std::string wrapped =
        unio_ui::license::codec::encode_wrapped(payload, signature);
